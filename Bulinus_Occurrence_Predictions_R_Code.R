...
# Set working directories
occurrence_points_path <- "C:/Users/DELL/Documents/Schist_Points/Bulinus_Points_5km_rarefied_points.csv"
...
# Generate random points within the unsuitable area
  pseudoabsence_points <- spatSample(unsuitable_area, size = 300, method = "random", na.rm = TRUE, xy = TRUE)
...
writeRaster(combined_raster, filename = "combined_model_Bulinus.tif", overwrite = TRUE)
# plot(combined_raster)

# Print final results
cat("\nModel weights (based on mean AUCs):\n")
print(model_weights)
cat("\nCombined model prediction saved as 'combined_model_Bulinus.tif'\n")
...
ggplot(all_response_data, aes(x = Value, y = Predicted_Probability)) +
  ...
  labs(
    ...
    title = "Response Curves of Prediction Bulinus spp. vs Environmental Variables"
  ) +
  theme_bw() +
  theme(
...
output_file <- "Bulinus_future_SSP126.tif"
...
# Save the final ensemble suitability map
output_file <- "Bulinus_future_SSP585.tif"
...
Bulinus_ext_Data <- as.data.frame(final_data)
view(Bulinus_ext_Data)
# Save the extracted data to a CSV file
write.csv(Bulinus_ext_Data, "F:/Model_Outputs/Bulinus_Extract_Data.csv", row.names = FALSE)
##################################################################################################################
